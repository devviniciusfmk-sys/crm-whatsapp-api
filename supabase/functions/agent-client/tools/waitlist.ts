...
> = {
  ...
  description:
    "Put this customer on the waiting list for a time that is not available. CALLING THIS TOOL IS THE ONLY THING THAT PUTS ANYBODY ON ANY LIST — writing 'vou ficar de olho', 'te aviso se vagar' or 'anotei aqui' puts nobody on it, and leaves them waiting for a message that will never come, sure that they are covered. If you tell the customer you will let them know, you MUST call this tool in the same turn. Use it RIGHT AFTER telling them there is nothing free on the day they asked for, and only if they say yes to being told — offer it in the same message as the refusal ('não temos quarta; quer que eu te avise se vagar?'), because thirty seconds later they have gone somewhere else. NEVER use it when there IS a free time: offer the free time instead. NEVER promise a slot will appear, and never say how likely it is — you do not know, and a promise of a chair that never frees costs more than the refusal did. What you can say is that they are on the list and will hear from you if something opens. ALWAYS WRITE TO THE CUSTOMER IN THE LANGUAGE THEY ARE USING.",
